...
public:
     ...
     */
    ~Set();
    QList<QPair<T, int> > *setToList(); /// Делает из дерева список.
private:
    /*!
     * \brief Ячейка дерева
     */
    ...
    /*!
     * \brief setToList Вспомогательная рекурсивная функция.
     */
    void setToList(QList<QPair<T, int>> *list, TreeNode *currentNode);
};
...
template <typename T>
QList<QPair<T, int> > *Set<T>::setToList()
{
    QList<QPair<T, int>> *result = new QList<QPair<T, int>>();
    if (root != nullptr)
        setToList(result, root);
    return result;
}
...
template <typename T>
void Set<T>::setToList(QList<QPair<T, int> > *list, TreeNode *currentNode)
{
    if(currentNode->leftChild != nullptr)
        setToList(list, currentNode->leftChild);
    list->append({currentNode->value, currentNode->quantity});
    if(currentNode->rightChild != nullptr)
        setToList(list, currentNode->rightChild);
}
